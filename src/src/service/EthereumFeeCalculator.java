...
public class EthereumFeeCalculator implements FeeCalculator{
    ...
    public double calculateFees(FeeLevel feeLevel) {
        double gasPriceGwei;
        switch (feeLevel) {
           case ECONOMICAL : gasPriceGwei = GAS_PRICE_ECONOMICAL;
               break;
            case STANDARD : gasPriceGwei = GAS_PRICE_STANDARD;
                break;
            case FAST: gasPriceGwei = GAS_PRICE_FAST;
                break;
            default: throw new IllegalArgumentException("Unknown fee level: " + feeLevel);
        }
