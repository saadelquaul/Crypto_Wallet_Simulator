...
public class TransactionService {
    ...
    public ITransaction createTransaction(String sourceAddress, String destinationAddress, double amount, FeeLevel feeLevel
        ...
        Optional<IWallet> destinationWalletOpt = walletService.getWalletByAddress(destinationAddress);
        if (!destinationWalletOpt.isPresent()) {
            throw new InsufficientFundsException("Source wallet not found for address " + destinationAddress);
        }


        IWallet sourceWallet = sourceWalletOpt.get();
        IWallet destinationWallet = destinationWalletOpt.get();
